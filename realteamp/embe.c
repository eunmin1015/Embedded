...
int baseballgame(void){
  ...
  int ball; // 볼의 수를 세기 위한 변수 
	bitmainfunc("basestart.bmp");
	text("GAME START", "        ");
	
  srand((unsigned)time(NULL)); 
  for (i = 0; i < 3; i++) 
  { 
	temp = (rand() % 6) + 1; // temp변수에 현재 랜덤 값을 저장 
...
int setInit(void){
   ...
   if(ondo < 20) 
    { bitmainfunc("coldtowork.bmp"); text(temperature, "Cold to study");}
   else if(ondo >= 20 && ondo < 26)
    {bitmainfunc("goodtowork.bmp"); text(temperature, "Nice to study!");}
   else 
    {bitmainfunc("hottowork.bmp"); text(temperature, "Hot to study..!");}
